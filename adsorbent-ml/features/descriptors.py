...
- **Pore block** (CoRE properties on the exemplar match): LCD/PLD [Å],
  ...
  actually depends on. IZA matches carry no pore data in v1 (CIFs exist,
  no Zeo++ here) → NaN + ``pore_has_data=0``.
...
def build_feature_matrix(labels: pd.DataFrame, matched: pd.DataFrame, *,
    ...
    > class table. Pores: CoRE exemplar first, QMOF DFT geometry as flagged
    fallback (``pore_qmof=1``). Topologies: top-N one-hots + ``topo_other``;
    pass the training ``topologies`` list at inference for stable columns.
    """
